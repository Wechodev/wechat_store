...
class UploadFile {//类定义开始
    ...
    private function save($file) {
        ...
        if(($this->thumb || $this->cut) && in_array(strtolower($file['extension']),array('gif','jpg','jpeg','bmp','png'))) {
            $image =  getimagesize($filename);
            if(false !== $image) {
            	//import($this->imageClassPath);
            	if($this->cut){
	                //是图像文件生成缩略图
	                $cutWidth		=	explode(',',$this->cutWidth);
	                $cutHeight	=	explode(',',$this->cutHeight);
	                $cutPrefix	=	explode(',',$this->cutPrefix);
	                $cutSuffix    =   explode(',',$this->cutSuffix);
	                $cutFile		=	explode(',',$this->cutFile);
	                $cutPath      =   $this->cutPath?$this->cutPath:dirname($filename).'/';
	                $cutExt       =   $this->cutExt ? $this->cutExt : $file['extension']; //自定义裁剪图扩展名
	                // 生成图像缩略图
	                
	                for($i=0,$len=count($cutWidth); $i<$len; $i++) {
	                    if(!empty($cutFile[$i])) {
	                        $cutname  =   $cutFile[$i];
	                    }else{
	                        $prefix     =   isset($cutPrefix[$i])?$cutPrefix[$i]:$cutPrefix[0];
	                        $suffix     =   isset($cutSuffix[$i])?$cutSuffix[$i]:$cutSuffix[0];
	                        $cutname  =   $prefix.basename($filename,'.'.$file['extension']).$suffix;
	                    }
	                    Image::cut($filename,$cutPath.$cutname.'.'.$cutExt,$cutWidth[$i],$cutHeight[$i]);                    
	                }
            	}
            	...
            	if($this->thumb){
			        ...
			        if(!is_dir($thumbPath)) {
			            ...
			            if(is_dir(base64_decode($thumbPath))) {
			                $savePath	=	base64_decode($thumbPath);
			            }else{
			                // 尝试创建目录
    ...
    private function check($file) {
        ...
        if(!$this->checkExt($file['extension'])) {
            $this->error ='上传文件类型不允许,上传格式为：'.implode(",",$this->allowExts);
            return false;
        }
